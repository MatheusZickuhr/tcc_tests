...
a = FlappyBird()
teste = PLE(a, display_screen=False, force_fps=True)
teste.init()
teste.reset_game()

actions = teste.getActionSet()
...
class NeatAgent:
    ...
    def start_training(self):
        self.population = self.create_population(size=2)
        generations = 1000
        ...
        def calc_fitness(e):
            ...
            game = FlappyBird()
            p = PLE(game, display_screen=False, force_fps=True)
            p.init()
            p.reset_game()
        ...
        for i in tqdm(range(generations)):
            ...
            for e in self.population:
                thread = Thread(target=calc_fitness, args=(e,))
