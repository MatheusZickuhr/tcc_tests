...
ne_mem_usage = np.mean([float(ne_csv[i][0]) for i in range(1, len(ne_csv))])
neat_mem_usage = np.mean([float(neat_csv[i][0]) for i in range(1, len(neat_csv))])
dql_mem_usage = np.mean([float(dql_csv[i][0]) for i in range(1, len(dql_csv))])
